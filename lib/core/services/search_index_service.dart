...
class SearchIndexService {
  ...
  Future<Map<String, String>> getIndex({
    ...
    try {
      ...
      if (_enableValidationLogs && kDebugMode) {
        debugPrint(
            '📊 [SearchIndexService] Built index for $indexKey: ${index.length} hymns in ${duration.inMilliseconds}ms');
      }
    ...
    for (final hymn in hymns) {
      ...
      final searchableText = [
        ...
        hymn.englishTitleOld ??
            '', // English title (important for bilingual search)
      ...
      final normalized =
          AmharicPhoneticService.normalizeAmharic(searchableText);
  ...
  Future<void> prebuildIndex({
    ...
    if (_indexCache.containsKey(indexKey) ||
        _buildingIndexes.contains(indexKey)) {
      return;
    }
